							...
							<footer class="messageOptions clearfix">
								<nav>
									<ul class="smallButtons">
										...
										{if $message->userID != $__wcf->getUser()->userID}<li class="jsReportConversationMessage" data-object-id="{@$message->messageID}"><a title="{lang}wcf.conversation.message.report{/lang}" class="button jsTooltip"><img src="{icon}warning{/icon}" alt="" class="icon16" /></a></li>{/if}
